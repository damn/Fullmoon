...
(defn- defmodifier [k operations]
  (defc* k {:schema [:map-optional operations]}))
...
(defn defstat [k {:keys [modifier-ops effect-ops] :as attr-m}]
  ...
  (when effect-ops
    (let [effect-k (stat-k->effect-k k)]
      (defc* effect-k {:schema [:map-optional effect-ops]})
...
; is called :base/stat-effect so it doesn't show up in (:schema [:components-ns :effect.entity]) list in editor
; for :skill/effects
