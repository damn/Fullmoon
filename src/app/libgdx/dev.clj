(ns app.libgdx.dev
  ...
  How to use:
  lein run -m app.libgdx.dev ~app-namespace~ ~app-fn-with-no-args~

  Example:
  lein run -m app.libgdx.dev gdl.simple-test app
  ...
  ``` vimscript
  nmap <F5> :Eval (do (in-ns 'app.libgdx.dev)(restart!))
  ```"
...
(defn ^:no-doc dev-loop []
  ...
  (loop []
    (when-not @app-start-failed
      (do
       (println "refresh")
       (.bindRoot #'refresh-result (refresh :after 'app.libgdx.dev/dev-loop))
